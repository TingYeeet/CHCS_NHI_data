...
# 資料夾路徑
folder_path = "就診千分比對pm2.5"

# 建立輸出圖表資料夾
output_dir = "scatter_plots_by_year"
...
for file_name in os.listdir(folder_path):
    if file_name.endswith(".csv"):
        ...
        for year, group in df.groupby("year"):
            ...
            plt.title(f"{file_name[:-4]} ({year})：就診千分比 vs PM2.5")
            plt.xlabel("PM2.5")
            plt.ylabel("就診人數千分比 (‰)")
            ...
            # 儲存圖檔
            output_path = os.path.join(output_dir, f"{file_name[:-4]}_{year}_scatter.png")
            plt.savefig(output_path)
            plt.close()

print("每年圖表已完成並儲存在 scatter_plots_by_year 資料夾中。")
