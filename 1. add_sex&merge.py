...
# 讀取 Excel 檔案
excel_path = "./每週呼吸道疾病就醫人數/2016-2019 年每週呼吸道疾病就醫人數.xlsx"  # 修改為你的檔案名稱
...
for sheet_name in xls.sheet_names:
    ...
    # 聚合 case_c（合併 sex）
    grouped = df.groupby(['ID1_CITY', 'year', 'week'], as_index=False)['case_c'].sum()
...
for disease, dfs in disease_data.items():
    ...
    combined_df = combined_df.groupby(['ID1_CITY', 'year', 'week'], as_index=False)['case_c'].sum()
    combined_df.to_csv(f"./周-呼吸道疾病就醫人數/{disease}.csv", index=False, encoding='utf-8-sig')
